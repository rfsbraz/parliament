...
- Comprehensive coded value translation (types, states, scopes)
...
from database.translators import OrcamentoEstadoTranslator
...
class OrcamentoEstadoMapper(SchemaMapper):
    ...
    Includes coded value translation for types, states, and workflow elements.
    ...
    - Comprehensive coded value translations
    """
    
    def __init__(self, session):
        super().__init__(session)
        # Initialize translator for coded values
        self.translator = OrcamentoEstadoTranslator()
        ...
        try:
            ...
            if format_type == 'legacy':
                ...
                for proposta in proposal_elements:
                    try:
                        success = self._process_legacy_proposal(proposta, legislatura, filename)
                        results['records_processed'] += 1
                        if success:
                            results['records_imported'] += 1
                            self.processed_proposals += 1
                    except Exception as e:
                        error_msg = f"Legacy proposal processing error in {filename}: {str(e)}"
                ...
                for item in item_elements:
                    try:
                        success = self._process_current_item(item, legislatura, filename)
                        results['records_processed'] += 1
                        if success:
                            results['records_imported'] += 1
                            self.processed_items += 1
                    except Exception as e:
                        error_msg = f"Current item processing error in {filename}: {str(e)}"
                        logger.error(error_msg)
                        results['errors'].append(error_msg)
                        if strict_mode:
                            raise
            ...
            logger.info(f"Statistics: {self.processed_proposals} proposals (legacy), "
    ...
    def _detect_format_type(self, xml_root: ET.Element, filename: str) -> str:
        ...
        logger.warning(f"Could not definitively determine format type for {filename}, defaulting to current")
        return 'current'
    ...
    def _extract_legislatura_from_path(self, file_path: str) -> Optional[Legislatura]:
        ...
        legislatura = self.session.query(Legislatura).filter_by(numero=legislatura_num).first()
        if not legislatura:
    ...
    def _process_legacy_proposal(self, proposta: ET.Element, legislatura: Legislatura, filename: str) -> bool:
        """
        ...
        try:
            ...
            proposal_id = DataValidationUtils.safe_float_convert(self._get_text_value(proposta, 'ID'))
            ...
            tipo_desc = self.translator.tipo_proposta_alteracao_legacy(tipo)
            estado_desc = self.translator.estado_proposta_legacy(estado)
            incide_desc = self.translator.incide_proposta_alteracao(incide)
            ...
            existing = self.session.query(OrcamentoEstadoPropostaAlteracao).filter_by(
            ...
            if existing:
                ...
                existing.incide_desc = incide_desc
                existing.tipo = tipo
                existing.tipo_desc = tipo_desc
                existing.estado = estado
                existing.estado_desc = estado_desc
    ...
    def _process_current_item(self, item: ET.Element, legislatura: Legislatura, filename: str) -> bool:
        """
        ...
        try:
            ...
            item_id = DataValidationUtils.safe_float_convert(self._get_text_value(item, 'ID'))
            id_pai = DataValidationUtils.safe_float_convert(self._get_text_value(item, 'ID_Pai'))
            ...
            # Translate coded values
            tipo_desc = self.translator.tipo_item_orcamento(tipo)
            estado_desc = self.translator.estado_proposta_current(estado)
            ...
            existing = self.session.query(OrcamentoEstadoItem).filter_by(
                item_id=int(item_id)
            ).first()
    ...
    def _process_legacy_nested_data(self, proposta: ET.Element, proposal_obj: OrcamentoEstadoPropostaAlteracao):
        ...
        try:
            ...
            logger.debug(f"Processed legacy nested data for proposal {proposal_obj.proposta_id}")

        except Exception as e:
            logger.error(f"Error processing legacy nested data: {e}")
            raise
    
    def _process_current_nested_data(self, item: ET.Element, item_obj: OrcamentoEstadoItem):
        ...
        try:
            ...
            if artigos_elem is not None:
                for artigo_elem in artigos_elem.findall('Artigo'):
                    artigo_id = DataValidationUtils.safe_float_convert(self._get_text_value(artigo_elem, 'ID_Art'))
                    id_pai = DataValidationUtils.safe_float_convert(self._get_text_value(artigo_elem, 'ID_Pai'))
            ...
            if propostas_elem is not None:
                for proposta_elem in propostas_elem.findall('Proposta'):
                    proposta_id = DataValidationUtils.safe_float_convert(self._get_text_value(proposta_elem, 'ID_PA'))
                    id_pai = DataValidationUtils.safe_float_convert(self._get_text_value(proposta_elem, 'ID_Pai'))
                    ...
                    if proposta_id:
                        ...
                        # Translate coded values
                        tipo_desc = self.translator.tipo_proposta_alteracao_legacy(tipo)  # Use legacy for consistency
                        estado_desc = self.translator.estado_proposta_current(estado)
                        incide_desc = self.translator.incide_proposta_alteracao(incide)
